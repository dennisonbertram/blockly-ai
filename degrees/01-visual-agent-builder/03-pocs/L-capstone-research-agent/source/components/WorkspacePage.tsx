'use client'
/**
 * WorkspacePage.tsx — L-capstone (extended from L5)
 *
 * Main client component: Blockly workspace + Run button + Load Demo button + output pane.
 *
 * Architecture:
 ...
 *
 * Capstone extension: "Load Demo" button loads public/demo-program.json into the workspace.
 *
 * Data flow:
 * 1. User arranges blocks in BlocklyWorkspace (or clicks "Load Demo").
...
export function WorkspacePage() {
  ...
  const [loadDemoState, setLoadDemoState] = useState<'idle' | 'loading' | 'loaded' | 'error'>('idle')
  /** Ref to trigger workspace reload from the outside */
  const loadJsonRef = useRef<((json: Record<string, unknown>) => void) | null>(null)
  ...
  const handleLoadDemo = useCallback(async () => {
    setLoadDemoState('loading')
    try {
      const response = await fetch('/demo-program.json')
      if (!response.ok) {
        throw new Error(`Failed to load demo: ${response.status}`)
      }
      const demoJson = (await response.json()) as Record<string, unknown>
      // Trigger workspace reload if ref is registered
      if (loadJsonRef.current) {
        loadJsonRef.current(demoJson)
      }
      setWorkspaceJson(demoJson)
      setLoadDemoState('loaded')
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      setError(`Failed to load demo: ${message}`)
      setLoadDemoState('error')
    }
  }, [])
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        <button
          ...
          disabled={isRunning || loadDemoState === 'loading'}
          style={{
            padding: '10px 24px',
            backgroundColor: loadDemoState === 'loaded' ? '#2196F3' : '#6c757d',
            color: 'white',
            border: 'none',
            borderRadius: '4px',
            fontSize: '16px',
            cursor: (isRunning || loadDemoState === 'loading') ? 'not-allowed' : 'pointer',
            fontWeight: 'bold',
          }}
          data-testid="load-demo-button"
        >
          {loadDemoState === 'loading' ? 'Loading...' : loadDemoState === 'loaded' ? 'Demo Loaded' : 'Load Demo'}
        </button>
        <button
          onClick={handleRun}
          disabled={isRunning}
