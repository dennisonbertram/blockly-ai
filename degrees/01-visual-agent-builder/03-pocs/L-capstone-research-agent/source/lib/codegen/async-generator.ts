/**
 * async-generator.ts — L-capstone (extended from L4)
 *
 * AsyncJavascriptGenerator: wraps the generated block code in an async ES module.
 * Extended from L4 to add __tools injection in the run() signature.
 *
 ...
 *
 * Capstone extension: run() now includes `tools: __tools` so tool bodies
 * can call `await __tools.search(input.query)` and `await __tools.fetch(url)`.
 * The executor (run-emitted.ts) injects the stub implementations.
 */
...
const RUN_SIGNATURE = `export default async function run({ model: __model_provider, sink: __sink, tools: __tools } = {}) {`
